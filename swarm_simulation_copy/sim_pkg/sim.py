...
class Simulator():
    def __init__(self, config_data, initfile):
        ...
        self.comm_radius, self.packet_success, self.num_msgs, self.msg_size = self.config_data[
            "COMM_RANGE"], self.config_data["PACKET_SUCCESS_PERC"], self.config_data["NUM_OF_MSGS"], self.config_data["MSG_SIZE"]
        self.arena_length, self.arena_height = self.config_data["LENGTH"], self.config_data["WIDTH"]
        self.time_async, self.rtf, self.sim_time_step, self.sim_time_max = self.config_data["TIME_ASYNC"], self.config_data[
            "REAL_TIME_FACTOR"], self.config_data["SIM_TIME_STEP"], self.config_data["SIM_TIME"]
    ...
    def initialize_swarm(self):
        ...
        if self.use_init == 1:
            ...
            # Remove .py ending from initfile name
            initfile = os.path.splitext(self.initfile)[0]
            ...
            x, y, theta, a_ids = np.random.uniform(-(self.arena_length - 0.1) / 2, (self.arena_length - 0.1) / 2, size=self.num_robots), np.random.uniform(-(
                self.arena_height - 0.1) / 2, (self.arena_height - 0.1) / 2, size=self.num_robots), np.zeros(self.num_robots), np.zeros(self.num_robots)

        if self.time_async == 1:
            # rand does 0 to 1 range automatically
            clocks = np.random.rand(self.num_robots) * 0.001
        ...
        self.swarm[:] = [Robot(i, x[i], y[i], theta[i], a_ids[i],
                               clocks[i], self.num_robots) for i in range(self.num_robots)]
    ...
    def launch(self, vis, gui):
        ...
        try:
            ...
            first = True
            # This will be some value initially, but will change slightly over time
            actual_rtf = self.rtf
            ...
            while not self.stop_sim:
                ...
                # Prevent simulation from starting until all robot clients are connected to the server
                if self.bot_server.num_connected < self.num_robots:
                    continue
                ...
                if vis == 1:
                    ...
                    if delta_vis > 0.05:
                        ...
                        delta_vis, actual_rtf_sum, iteration_count = 0, 0, 0
                        gui.update(self.swarm, real_time,
                                   self.sim_time, self.rtf)
    ...
    def update_state(self, data):
        ...
        if function == 1:  # set_LED
            ...
            params = data["params"]
            self.swarm[robot_id].velocity = (
                params[0] / 100) * motor_full_speed, (params[1] / 100) * motor_full_speed
            ...
            for robot in self.swarm:
                if robot.id != robot_id:
                    dist = (sender_posn[0] - robot.posn[0])**2 + \
                        (sender_posn[1] - robot.posn[1])**2
                    if dist < comm_dist:
                        # generate a random val and check if it's less than packet_success
                        if np.random.uniform() < self.packet_success:
    ...
    def integrate_world(self, dt):
        ...
        for i in range(self.num_robots):
            ...
            # If new pos is diff from old pos:
            if robot.posn[0] != pos[1] or robot.posn[1] != pos[2]:
                ...
                for r in np.where(robot.collision_list <= 0)[0]:
                    if r != robot.id:  # Avoid self-collision checks
                        ir_dist = (
                            self.swarm[r].posn[0] - pos[1])**2 + (self.swarm[r].posn[1] - pos[2])**2
                        ...
                        vel1, vel2 = (self.swarm[r].velocity[0])**2 + (
                            self.swarm[r].velocity[1])**2, (robot.velocity[0])**2 + (robot.velocity[1])**2
                        max_v = max(vel1, vel2)  # Select larger velocity
                        # time to collision
                        robot.collision_list[r] = ir_dist / \
                            (2 * np.sqrt(max_v) * dt)

                if not collision_will_occur:
                    robot.posn[0], robot.posn[1] = min(max(pos[1], -self.arena_length / 2 + self.robot_radius), self.arena_length / 2 - self.robot_radius), min(
                        max(pos[2], -self.arena_height / 2 + self.robot_radius), self.arena_height / 2 - self.robot_radius)
            robot.posn[2], robot.clock = pos[0], max(
                robot.clock, self.sim_time)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-i", "--initfile", type=str,
                        help="Path to initialization file", required=False)  # default value: None
